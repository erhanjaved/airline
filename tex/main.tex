...
Crew-Hour Availability. Each fleet type $j$ has $N_j$ aircraft available, each with a maximum of $H_{\max}$ crew-hours per day.
Total block hours assigned to fleet $j$ cannot exceed the total available crew-hours:
$$
\sum_{i \in F} h_i \, x_{i,j} \leq N_j \cdot H_{\max} \quad \forall j \in K
$$
...
Each fleet type $j$ has $N_j$ aircraft available, each with a maximum of $H_{\max}$ crew-hours per day.
Total block hours assigned to fleet $j$ cannot exceed the total available crew-hours:
\begin{align*}
\sum_{i \in F} h_i \, x_{i,j} \leq N_j \cdot H_{\max} \quad \forall j \in K
\end{align*}
...
The revenue and cost terms in the objective function are largely independent, which results in trivial solutions. Introducing overbooking penalties could create a stronger coupling between seat allocation and fleet assignment.
