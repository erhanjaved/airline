...
\begin{itemize}
    ...
    \item Number of PrimeFly seats sold: $N_{Eco}$
...
\noindent Where $P_i$ is the price of the seats, and $N_i$ is the number of the seats sold.
...
\noindent $C$ is the total seat capacity, and $C_r$ and $C_d$ are the capacity of regular seats and discount seats respectively.
...
\subsubsection{Introduction to Weighted Sum Method}
The weighted sum method is used for multi-objective optimization formulation \cite{article1}.
\begin{equation}
    \text{Minimize} \hspace{1em} J(x,p) 
\end{equation}
\begin{align*}
    \hspace{6em}\text{Subject to} \hspace{1em} g(x,p) &\leq 0\\
     h(x,p) &= 0\\
    x_{i,LB} \leq x_i &\leq x_{i,UB} \hspace{1em} (i = 1,\dots,n) 
\end{align*}

\noindent $J(x,p)$ is the vector valued objective function. $x$ and $p$ are a design vector and a fixed parameter vector respectively.

Aggregated objective function
\begin{equation}
J_{\text{weighted sum}} = w_1J_1 + w_2J_2 + \dots + w_mJ_m
\end{equation}

\subsubsection{Objective Function}
We adopt the notation from Sections 2.1 and 2.2.
\begin{align}
...
That said, our model does not consider network flow or repositioning of aircraft, which means that we assume there is sufficient fleet at each origin where needed. Although we included pairing solutions to make the fleet return to where it departed as a priority, it does not mean that our model works on a timed schedule. In addition, as shown in the sample data section (3.1.4), the model requires a list of flight legs with fixed demand estimates and fleet information. 
